...
export class GeminiLiveClient {
  ...
  async connect() {
    ...
    const isMale = this.voiceName.includes("male");
    const genderText = isMale ? "男性" : "女性";
    const isDefault = this.voiceName.startsWith("default");

    const systemInstructionText = `你是一个本地 AI Agent。
请务必使用标准、纯正、地道的中文普通话进行语音对话。你的发音应当自然、流畅，表现得像一个土生土长的中国${genderText}，严禁带有任何不自然的“外国口音”或机械感。如果你检测到当前是${genderText}发音，请保持稳定。
